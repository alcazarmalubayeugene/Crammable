...
The schema has 11 tables (`profiles`, `decks`, `flashcards`, `quiz_sessions`, `quiz_answers`, `payment_submissions`, `referral_events`, `app_reviews`, `bug_reports`, `rate_limit_log`, `admin_action_log`) and ~20 functions. **Use the functions — do not re-implement their logic in TypeScript.** (`schema.sql` is the canonical, authoritative list — count is approximate here and grows over time.)

Recent additions (v.17): `quiz_sessions.is_public` + the `get_public_quiz_result()` SECURITY DEFINER RPC (shareable quiz results — narrow public projection, anon-callable), and `profiles.theme_preference` / `font_size_preference` / `font_pair_preference` (theme/font sync; freely user-updatable, not guarded by `protect_immutable_profile_fields()`). Admin manual Pro revocation: the `revoke_pro()` SECURITY DEFINER RPC + the `revoke_pro` `admin_action_log` action value (see §5). **(v.20)** Two function updates (both migrated live): `check_referral_cap()` now enforces a **lifetime-1 cap on `signup`** (was monthly-5); `submit_quiz_result()` now grades **identification** answers by **substring/contains match** in either direction (multiple-choice stays exact-match). `quizType` threaded through `SubmitQuizAnswer` → `LocalAnswer` → `PausedQuizState.answers`. New routes: `GET /api/admin/status` (DB+DeepSeek+App liveness, admin-gated) and `GET /api/rewards/referrals` (referral history enriched with referred user's first name via service-role). **(BackEnd branch — reliability hardening):** `recompute_deck_card_count(p_deck_id uuid)` SECURITY INVOKER RPC (migration applied) replaces the two-query count+update in manual card CRUD with a single atomic transaction — use this via `recomputeDeckCardCount()` in `src/lib/db/flashcards.ts`, never re-implement inline. `GET /api/profile` route added (`ProfileResult` in contracts, handler at `src/app/api/profile/route.ts`) — all pages must fetch profile via `fetch(ApiPaths.profile)`, never via a direct browser Supabase query. Living Deck catch block now logs unexpected errors (not `INSUFFICIENT_CREDITS`/`RATE_LIMITED`) instead of silently swallowing them. `POST /api/admin/payments/reject` validates `rejectionReason` and `notes` length server-side and returns `VALIDATION_ERROR` if exceeded — no silent truncation. Quiz explanation cache and paused quiz state use `localStorage` (survive tab close/refresh); `fetchExplanation` has a 12 s `AbortController` timeout. **(v.21 — MC distractor quality):** `flashcards.distractors text[] NOT NULL DEFAULT '{}'` column added (migration `add_distractors_to_flashcards` applied). DeepSeek generation prompts (both standard/deep-dive and reinforcement) now request 3 AI-crafted plausible-but-wrong MC options per card alongside a tightened `back` answer (≤20 words, single sentence). `GeneratedCard` and `Flashcard` both carry `distractors: string[]`. Both write-path RPCs (`create_deck_with_cards_and_charge`, `insert_reinforcement_cards_and_charge`) extract `distractors` from the JSONB input. The quiz route (`POST /api/quiz/[id]`) uses `card.distractors` directly when ≥3 entries exist; falls back to cross-card `back` sampling for pre-v.21 cards with an empty array.

**(BackEnd branch — deck archive, bug reports, multi-PDF):** Three additions, all pending live-DB migration (schema.sql updated, not yet applied to the remote project as of this writing — see PR notes). **Deck archive:** `decks.archived_at` (nullable timestamp) added; `setDeckArchived()` toggles it via `POST`/`DELETE /api/decks/[id]/archive`; `countDecksForUser()` (the `maxDecks` cap check) and the default `GET /api/decks` list now filter `archived_at IS NULL` — pass `?archived=1` to list the archived view instead. Dashboard has an active/archived toggle and an Archive/Unarchive deck-menu action. **Bug reports:** new `bug_reports` table (modeled on `app_reviews`, no one-per-user cap) + `verify_bug_report()` SECURITY DEFINER RPC (mirrors `verify_app_review()`, gated by `check_referral_cap()`'s new `bug_report` monthly-cap-3 branch since multiple reports per user are allowed). New reward: `ReferralCaps.bug_report` (+10 Capycoins, up to 3/month). Routes: `POST /api/rewards/submit-bug-report`, `GET /api/admin/bug-reports`, `POST /api/admin/bug-reports/verify`. UI: a "Report a bug" form on `/rewards`, a "Pending Bug Reports" queue on `/admin`. **Multi-PDF upload:** `POST /api/upload` now accepts multiple `file` fields (≤ `MAX_FILES_PER_UPLOAD`, 5) and returns `MultiUploadResult { files: PerFileUploadResult[] }` instead of a single top-level result — every caller of this route needed updating for the shape change. `/api/generate` is untouched: the frontend concatenates all-text-path files into one `extractedText` blob (with `=== filename ===` section headers) before calling it once, same as a single-file upload always did. Scope boundary: multi-file upload only supports the text-extraction fast path — if any file in a multi-file batch needs OCR (scanned/image PDF), the batch is rejected with a message asking the user to upload that file alone; the existing single-file OCR/Tesseract/paste-fallback pipeline is completely unchanged and still the only path for scanned documents.
...
| Read/write `card_count` in two separate queries | Call `recompute_deck_card_count()` RPC (atomic) |
| Browser Supabase direct profile read (`supabase.from(profiles)` in a client component) | `fetch(ApiPaths.profile)` — cookie-authed server route |
| Bare `catch {}` swallowing all errors in a background task | Log unexpected errors; only silence known codes (`INSUFFICIENT_CREDITS`, `RATE_LIMITED`) |
