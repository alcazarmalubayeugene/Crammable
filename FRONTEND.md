...
> **Google OAuth (external, no env needed).** "Continue with Google" depends on
> dashboard-side config, not on `.env.local` — there are no Google secrets in the repo or
> env (the Client Secret lives only in Supabase). It's already configured on the shared
> Supabase project, so a fresh clone works out of the box. If you ever spin up a *new*
> Supabase project, redo the one-time setup (Google Cloud OAuth client + enable the Google
> provider + redirect allow-list) per `docs/PROJECT-DOCUMENTATION.md` §5.
...
| v.19 | **Clickable "Crammable" wordmark across authenticated app pages.** The shared `<Navbar>` already supported a `wordmarkHref` prop (rendering the wordmark inside an `<a>` instead of a plain `<span>`), but most logged-in pages never passed it, so the top-left wordmark was dead text. Added `wordmarkHref={Routes.dashboard}` on `/dashboard`, `/decks/[id]`, `/admin`, `/settings`, `/settings/avatar`, and `/help` so clicking the wordmark returns the user to the dashboard. **Deliberately left unchanged:** public/shared pages (`/public/decks/[id]`, `/results/[sessionId]`) and auth pages (`/forgot-password`) already link the wordmark to `Routes.home` (correct for logged-out visitors); the `/settings?mode=reset-password` recovery view stays plain since the user may not be authenticated there; pages that hide the wordmark entirely (`/rewards`, `/decks/new`, the quiz screens) have nothing to link. No `Navbar.tsx` change needed — only call-site props. `npx tsc --noEmit` clean. |
| v.18 | **Google login/signup shipped.** Turned the long-standing disabled "Continue with Google" placeholder into a real Supabase Google OAuth (PKCE) flow. New `signInWithGoogle()` helper in `src/lib/supabase/browser.ts` (uses the existing cookie-based `@supabase/ssr` browser client so the PKCE verifier rides in a cookie the server callback can read; carries intent via `?consent=1&flow=…` on `redirectTo`). The `AuthCard.tsx` button is now live — in **signup mode it's gated on the existing DeepSeek consent checkbox** (RA 10173: no document reaches AI without consent), with a real multicolor Google mark + "Redirecting…" state. `src/app/api/auth/callback/route.ts` gained an **OAuth branch** (no `type` + provider `google`): persists `consent_deepseek = true` when `?consent=1` (admin client), prefills `full_name` from the Google profile, and routes new/incomplete profiles → `/onboarding/name` vs returning users → `/dashboard` (mirrors the dashboard's own onboarding check). For the edge where a brand-new user clicks the **login-mode** Google button (no checkbox shown), `src/app/onboarding/name/page.tsx` shows a **conditional consent checkbox** (only when `consent_deepseek` is still false) and prefills the name; `onboarding/course/page.tsx` + the `claim-profile-complete` route persist that consent — backstopped by the existing `CONSENT_REQUIRED` gate on `/api/generate`. `contracts.ts` gained `Routes.onboardingName/Course/Referral` and an optional `consentDeepseek` on `ClaimProfileCompleteRequest`. **Requires external setup** (one-time, dashboard-side, not in repo/env): a Google Cloud OAuth 2.0 Web client + enabling the Google provider in Supabase + the `/api/auth/callback` redirect allow-list — see `docs/PROJECT-DOCUMENTATION.md` §5. `npx tsc --noEmit` clean, `next build` green, no new lint errors. |
...
> endpoints with **zero backend changes**. **Google OAuth is now live (v.18)** — the
> formerly-disabled "Continue with Google" button runs a real Supabase PKCE flow
> (`signInWithGoogle()` helper, signup-mode button gated on the consent checkbox, an
> OAuth branch in `/api/auth/callback` that persists consent + prefills the name + routes
> new users into onboarding). It needs one-time external setup (Google Cloud OAuth client
> + Supabase provider config) — see the v.18 row above and `docs/PROJECT-DOCUMENTATION.md`
> §5. The Apple button and the phone-number field were removed from `AuthCard.tsx` on
> 2026-06-24 (keep Google only), so the SMS-gateway / phone-provider work that used to be
...
>
> **Update (2026-06-24): Google OAuth live — now on v.18.** The disabled "Continue with
> Google" placeholder on `AuthCard.tsx` is now a real Supabase Google OAuth (PKCE) flow.
> New `signInWithGoogle()` in `src/lib/supabase/browser.ts`; the signup-mode button is
> gated on the DeepSeek consent checkbox and carries the decision via `?consent=1` on the
> `redirectTo`. `src/app/api/auth/callback/route.ts` gained an OAuth branch: persists
> `consent_deepseek` (RA 10173), prefills `full_name` from the Google profile, and routes
> new/incomplete profiles into `/onboarding/*` vs returning users to `/dashboard`.
> `onboarding/name` now prefills the name and shows a conditional consent checkbox for the
> login-button edge (a never-registered user clicking "Sign in with Google"), with
> `onboarding/course` + `claim-profile-complete` persisting it; `/api/generate`'s existing
> `CONSENT_REQUIRED` gate is the backstop. `contracts.ts`: added `Routes.onboarding*` and
> an optional `consentDeepseek` on `ClaimProfileCompleteRequest`. **This is the only part
> that needs external, dashboard-side setup** (Google Cloud OAuth client + Supabase Google
> provider + the `/api/auth/callback` redirect allow-list) — documented in
> `docs/PROJECT-DOCUMENTATION.md` §5. Tested end-to-end against the live project (signup
> consent-gate, new-user→onboarding with name prefill, returning-user→dashboard).
> `tsc`/`build` green, no new lint errors.
...
> **Update (v.18):** ✅ **Google OAuth** — now live (real Supabase PKCE flow,
> consent-gated signup, callback OAuth branch + onboarding integration; the only thing it
> needs is the one-time external Google Cloud / Supabase dashboard config, see the v.18
> row). No longer an open item.
>
> Still genuinely open after v.18:
- **Google OAuth** ✅ **done (v.18)** — kept here only as a pointer. The Apple OAuth
  button and the phone-number (`+63` selector + input) field were **removed** from
  `AuthCard.tsx` (2026-06-24) — only "Continue with Google" remains, so the SMS gateway /
  phone-provider work is no longer needed.
