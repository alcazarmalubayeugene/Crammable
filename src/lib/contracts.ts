...
export interface Flashcard {
  ...
  front:            string;
  back:             string;
  explanation:      string | null;    // "why" lesson; null = card predates this column → live fallback
  distractors:      string[];         // AI-crafted MC wrong options; empty for pre-v.21 cards → falls back to cross-card selection
  tags:             string[];
...
export interface GeneratedCard {
  front:       string;
  back:        string;
  explanation: string;  // generated once with the card, reused free on future wrong answers
  distractors: string[]; // 3 AI-crafted plausible-but-wrong MC options; empty for pre-v.21 cards
  tags:        string[];
...
export interface QuizQuestion {
  ...
  correctExplanation: string | null;  // baked-in "why"; null → frontend calls ApiPaths.explainAnswer
}
...
// Live fallback for cards generated before the baked-in explanation existed.
